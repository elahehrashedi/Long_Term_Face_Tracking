function VTWVerification2( videoName, start_time, px1, py1, px2, py2,...
...
posCount = 1;
positionMat(posCount,:)= [ round(frameNumber), dx1, dy1, dx2-dx1, dy2-dy1];
posCount = posCount +1;
...
%                     myframe = imresize(myframe, [floor(fs(1)*0.5),floor(fs(2)*0.5)]);
%                 end
                %myframe = imresize(myframe, [floor(fs(1)*0.7),floor(fs(2)*0.7)]);
                img_files{i} = myframe;
        ...
        for k=1:1: size(t_boxes,1)%tracking_frames
            ...
            %fprintf(posFile, '%d %d %d %d %d\n' , frameNumber+k, x, y, h, w);
            positionMat(posCount,:)= [round(frameNumber+k), x, y, h, w];
            posCount = posCount +1;
            ...
            %imwrite(img_files{k}(tracked_x:tracked_x2,tracked_y:tracked_y2,:),imgVeri);
            %pause (0.1);
        end
        ...
        if Score_cos > similarity_threshold
            ...
            % calculate back tracking time
            % backtime = (saved_tracking_frames - size(t_boxes,1)) / myvideo.FrameRate  ; %second
            backtime = 0 ;
            if ((myvideo.currenttime - backtime ) < myvideo.duration)
                myvideo.currenttime = myvideo.currenttime - backtime;% +tp_tracking;
            else
        ...
        if ~isempty(boxes)
            ...
            for count = 1:size(v)
                if (I == 1)
                    ...
                    positionMat(posCount,:)= [round(frameNumber), y1(count)-pad, x1(count)-pad, (y2(count)-y1(count)), x2(count)-x1(count)];
                    posCount = posCount + 1;
                    do_tracking = 1;
                    verified_flag = 1;
                    %myframe = readFrame(myvideo);
