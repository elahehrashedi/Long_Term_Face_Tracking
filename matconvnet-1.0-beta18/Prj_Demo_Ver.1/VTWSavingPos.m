function [ frame_box ] = VTWSavingPos( videoName, start_time, px1, py1, px2, py2,...
    ...
    addpath(genpath('D:\Wayne Courses\Winter 2016\Project\FaceRetrieval'));
...
%%  
    ...
    posCount = 1;
    positionMat(posCount,:)= [ frameNumber, dx1, dy1, dx2-dx1, dy2-dy1];
    posCount = posCount +1;
        ...
        if (do_tracking == 1 )
            ...
            %just for show
                for k=1:1:tracking_frames
                        ...
                        positionMat(posCount,:)= [frameNumber+k, x, y, h, w];
                        posCount = posCount +1; 
           ...
           %% feature extraction for each detected face 
            ...
            for count = 1:size(v)
                if (I == 1)
                    ...
                    positionMat(posCount,:)= [frameNumber, y1(count)-pad, x1(count)-pad, (y2(count)-y1(count)), x2(count)-x1(count)];
                    posCount = posCount + 1;
                    do_tracking = 1;
                    verified_flag = 1;
                    %myframe = readFrame(myvideo);
...
%     end
fclose(posFile);
save('Positions.mat','positionMat');
disp('done!!!');
