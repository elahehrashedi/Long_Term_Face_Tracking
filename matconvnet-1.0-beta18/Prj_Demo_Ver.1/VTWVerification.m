...
%vgg_net=load('vgg_face_cut_fc7.mat');
...
numofframes = round(video_duration * frame_rate) ;
...
if isempty(dbox)
    ...
    pos_second_half = [];
    posCount = 1;
    pos_first_half_count = 0;
    pos_second_half_count = 0;
    ...
    pos_second_half(pos_second_half_count,:)= [ frameNumber, dx1, dy1, dx2-dx1, dy2-dy1];
    %pos_second_half_count = pos_second_half_count +1;
    do_tracking = 1;
    pad = 10;
...
%                 end
            ...
            for k=1:1: size(t_boxes,1)%tracking_frames
                ...
                if tracked_x2 > frame_size(1)
                    tracked_x2 = frame_size(1)-3;
                end
                if tracked_y2 > frame_size(2)
                    tracked_y2 = frame_size(2)-3;
                end
                temp_img = img_files{k}(tracked_x:tracked_x2,tracked_y:tracked_y2,:);
                if ~isempty(double(temp_img))
                      imwrite(temp_img,imgVeri);
                      if is_first_half
            ...
            if (first_run ==1) %feature extraction
                ...
                tracked_Images (:,:,:,k+1) = imresize(Image, [width,lenght]);
                
                Image_query = tracked_Images; % 5+1 images
            ...
            elseif (last_frame == saved_tracking_frames-1 )
                myframe = img_files{(last_frame)};
            elseif (last_frame == numofframes-1)
                myframe = img_files{(last_frame)};
            else
            ...
            if ~(xx2-xx1<1 || yy2-yy1 <1)
                ...
                if Score_cos > similarity_threshold
                    ...
                    if ((myvideo.currenttime) < myvideo.duration)
                        ...
                        myvideo.currenttime = 20/myvideo.framerate ;
            ...
            myframe = readFrame(myvideo);
            frameNumber = floor(myvideo.currenttime * myvideo.FrameRate);
            ...
            if ~isempty(boxes)
                ...
                for xx = 1:boxes_size(1)
                    tempx1=x1(xx)-round(pad/2);
                    tempy1= y1(xx)-round(pad/2);
                    tempx2 = x2(xx)+round(pad/2);
                    tempy2= y2(xx)+round(pad/2);
                    if (tempx1 < 0)
                        tempx1 = 3;
                    end
                    if (tempy1 <0)
                    ...
                    if tempx2 > frame_size(1)
                        tempx2 = frame_size(1)-3;
                    end
                    if tempy2 > frame_size(2)
                        tempy2 = frame_size(2)-3;
                    end
...
if (~isempty(pos_first_half))
    positionMat = [pos_first_half; pos_second_half];
else
    positionMat = pos_second_half;
            ...
            if frame~=1
                ...
                if (~first_time_track)
                    ...
                    if (distance>7)
                        ...
                        if (y2org) > frame_size(1)
                            y2org = frame_size(1)-3;
                        end
                        if x2org > frame_size(2)
                            x2org = frame_size(2)-3;
                        end
                        
                        img=im(yorg:(y2org),xorg:(x2org),:);
