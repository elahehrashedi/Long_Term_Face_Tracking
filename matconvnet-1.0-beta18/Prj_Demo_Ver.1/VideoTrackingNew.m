function [ frame_box ] = VideoTrackingNew( videoName , start_time , tracking_box ) % tracking_box x1, y1 , x2 , y2
    ...
    rectangle('Position',[y1+dy1, x1+dx1,dy2-dy1, dx2-dx1],'LineWidth',2,'EdgeColor','g');
        ...
        if (do_tracking == 1 )
            ...
            tracking_position = t_boxes (end,:) ; % change tracking position
             if (first_run ==1) %feature extraction
                ...
                %FeaCNN_query = FeaCNN_query';
                first_run = 0;
             end 
            %flag = KLT_tracking(myvideo,tracking_position, myvideo.currenttime,fgr,imgVerified);      
        ...
        if ~isempty(boxes)
           ...
           %% feature extraction for each detected face 
            ...
            for count = 1:size(v)
                if (I == 1)
                    disp('verified...');
                    %rectangle('Position',[y1(count)-pad,x1(count)-pad,(y2(count)-y1(count)),x2(count)-x1(count)],'LineWidth',2,'EdgeColor','b');
            ...
            if verified_flag == 0 
                do_tracking = 0 ;
                if (myvideo.currenttime+5 < myvideo.duration)
                    myvideo.currenttime = myvideo.currenttime+5;
                else 
                    stop_flag = 1;
                end 
